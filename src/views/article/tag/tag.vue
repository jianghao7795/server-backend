...
<script setup>
import { createTag, deleteTag, deleteTagByIds, updateTag, findTag, getTagList } from "@/api/tag";
...
const changeHide = (e) => {
  ...
  return new Promise((resolve, reject) => {
    updateTag({ ...e, status: e.status ? 0 : 1 }).then((resp) => {
      // console.log(e.ID);
      const primitive = { ...loading.value };
...
const enterDialog = async () => {
  ...
  switch (type.value) {
    case "create":
      ...
      break;
    case "update":
      res = await updateTag(formData.value);
      break;
    default:
      res = await createTag(formData.value);
      break;
